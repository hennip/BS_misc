...
M1<-"model{
  ...
  for (i in 1:N_FI){ #number of females in the Finnish data
    ...
    #thiam_obs[i]~dnorm(mean_thiam[year[i],stock[i]], 1000)T(0.01,)
    thiam_obs[i]~dlnorm(M_thiam[i], T_thiam)#dnorm(mean_thiam[year[i],stock[i]], 1/sd_thiam[year[i],stock[i]]^2,)T(0.01,)
    
    M_thiam[i]<-mean_thiam[year[i],stock[i]]-0.5/T_thiam
   
    S_YSFM[i]~dbeta(a_YSFM,b_YSFM) # survival from normal yolk-sac-fry mortality
