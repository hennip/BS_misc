...
load(file=paste0(pathMain,"WGBAST_shared/flhm/2024/output/FLHM_JAGS_2024_CR_coefDSHIER_data2024.RData")); trolling2<-T;Mname2<-"2024 CoefDS hierarchical"
...
summary(chains1[ ,regexpr("detect",varnames(chains))>0])

summary(chains[ ,regexpr("detect",varnames(chains))>0])

tbl<-array(NA, dim=c(37,5))
for( i in 1:37){
df<-summary(chains[,str_c("p.detect[",i,",1]")])
tbl[i,]<-df$quantiles
}

tbl<-cbind(Years, tbl)

colnames(tbl)<-c("Year","q2.5", "q25", "q50", "q75", "q97.5")

ggplot(tbl, aes(Year, group=Year))+
  theme_bw(base_size = 13)+
  geom_boxplot(
    aes(ymin = q2.5, lower = q25, middle = q50, upper = q75, ymax = q97.5),
    stat = "identity",fill=rgb(1,1,1,0.6))+
  ggtitle("p.detect, Tornionjoki")

# Kalix
tbl<-array(NA, dim=c(37,5))
for( i in 1:37){
  df<-summary(chains1[,str_c("p.detect[",i,",3]")]) # Chains1: Assessment model
  tbl[i,]<-df$quantiles
}
...
ggplot(tbl, aes(Year, group=Year))+
  ...
  geom_boxplot(
    aes(ymin = q2.5, lower = q25, middle = q50, upper = q75, ymax = q97.5),
    stat = "identity",fill=rgb(1,1,1,0.6))+
  ggtitle("p.detect, Kalix")+ylim(0,0.6)
